...
Once installed you will also need to create/update the *.Renviron* file in the root folder of your RStudio project to add the `DAC_TENANT` and `KEY_VAULT_NAME` keys to your R environment. These environment variables are automatically read by the code and tell it how to find the DAC Key Vault which stores all the configuration values used to connect. To open the *.Renviron* file for editing run then following command in the console from within your project (you may need to install the `usethis` package first):
...
#### Note
When the code is first run, a dialogue box may pop up asking to create a folder to save your
authentication credentials - please click *yes* if this occurs.

If you are having issues with your authentication try passing disabling the authentication cache with: `dac_odbc_connect(use_cache = FALSE)`
